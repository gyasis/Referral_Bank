...
def update():
    ...
    try:
        ...
        if referral:
            # Update only populated fields from the form
            update_values = [(key, request.form.get(key)) for key in request.form if key != "index" and request.form.get(key)]
            ...
            # Add new record
            columns = ', '.join([key for key in request.form if key != "index" and request.form.get(key)])
            placeholders = ', '.join(['?'] * len(columns.split(', ')))
            values = tuple([request.form.get(key) for key in request.form if key != "index" and request.form.get(key)])
