...
def filter_by_specialty():
    ...
    # Convert the dataframe to HTML, but without the 'id' column
    html_table = results[['NAME_OF_ORGANIZATION', 'NAME', 'SPECIALTY']].to_html(index=False, escape=False)
    
    # Add data-id attributes to each row
    row_template = '<tr data-id="{id}">{row_content}</tr>'
    modified_rows = [
        row_template.format(id=row['id'], row_content=row) for _, row in results.iterrows()
    ]
    
    # Replace original rows with modified rows
    for orig_row, mod_row in zip(re.findall(r"<tr>.*?</tr>", html_table, re.DOTALL), modified_rows):
        html_table = html_table.replace(orig_row, mod_row)
